...
ok(
  "toda linha tem menu de ações renderizado",
  kebabs === linhas,
  `${kebabs} menus para ${linhas} linhas`,
);
const opacidade = await p
  .locator('button[aria-label^="Ações de"]')
  .first()
...
ok(
  "aviso confirma a mudança pelo nome do custo",
  aviso.includes("Link dedicado"),
  aviso.split("\n")[0],
);
ok(
  "aviso oferece Desfazer",
...
const situacaoDepois = (
  await p
    ...
    .filter({ hasText: "Link dedicado" })
    .locator('[data-celula="situacao"]')
    .innerText()
).trim();
ok(
  "Desfazer restaura a situação anterior",
  situacaoDepois === situacaoAntes,
  `${situacaoAntes} → alterado → ${situacaoDepois}`,
);
...
ok(
  "gravar data avisa com a data em português",
  avisoData.includes("20/05/2027"),
  avisoData.split("\n")[0],
);
...
ok(
  "menu traz Editar, Duplicar, Dividir, Histórico e Excluir",
  rotulos.some((r) => r.includes("Editar")) &&
    rotulos.some((r) => r.includes("Duplicar")) &&
    rotulos.some((r) => r.includes("Dividir")) &&
    rotulos.some((r) => r.includes("Excluir")),
  rotulos.join(" | "),
);
await limparAviso();
...
ok(
  "item com lançamentos oferece 'Encerrar custo', não 'Excluir'",
  rotulosTotvs.some((r) => r.includes("Encerrar")) &&
    !rotulosTotvs.some((r) => r.trim() === "Excluir"),
...
ok(
  "a âncora é somente leitura",
  (await p.locator('input[name="pct_0"]').getAttribute("readonly")) !== null,
);
ok(
  "a âncora se identifica como quem absorve o resto",
...
ok(
  "TI cai automaticamente para 90% — sem chegar a 110%",
  tiDepois === "90,00",
  `TI = ${tiDepois}%`,
);
...
ok(
  "a âncora mostra quanto já cedeu",
  textoForm.includes("−15,50") || textoForm.includes("(−15,50)"),
  textoForm.split("\n").find((l) => l.includes("absorve")) ?? "",
);

console.log("\n  → cada percentual anda com o seu real");
const reaisComercial = await p
  .locator('input[aria-label*="Valor mensal de Comercial"]')
  .inputValue();
...
const alerta = await p
  ...
  .catch(() => "");
ok(
  "o erro nasce na linha culpada, com o quanto cabe",
  alerta.includes("Passou") && alerta.includes("%"),
  alerta,
);
const salvarHabilitado = await p
  .locator('form:has(select[name="setor_0"]) button[type="submit"]')
  .isEnabled();
...
const tres = await Promise.all(
  [0, 1, 2].map((i) => p.locator(`input[name="pct_${i}"]`).inputValue()),
);
ok(
  "três fatias iguais exibem 33,33% cada",
  tres.every((v) => v === "33,33"),
  tres.join(" / "),
);
...
ok(
  "admin aplica o rateio direto",
  avisoRateio.includes("aplicado"),
  avisoRateio.replace(/\n/g, " · "),
);
...
ok(
  "a página do custo mostra as duas fatias em % e em R$",
...
ok(
  "o histórico nomeia quem alterou",
  historico.includes("Ana Administradora"),
  historico.split("\n").slice(1, 3).join(" · "),
);
ok(
  "o histórico mostra o antes e o depois do campo",
  /Situação:\s*Em análise\s*→\s*Ativo/.test(historico.replace(/\s+/g, " ")),
  historico
    .split("\n")
    .filter((l) => l.includes("Situação"))
    .slice(0, 2)
    .join(" | "),
);
...
// A aba padrão é "Recorrente", então o investimento de TI não entra na conta —
// e é isso mesmo: são perguntas diferentes, e a aba diz qual está sendo feita.
ok(
  "gestor vê SÓ os custos da própria área",
  meus === SEMENTE.recorrentesDeTI,
  `${meus}, esperado ${SEMENTE.recorrentesDeTI}`,
);
const forasteiro = await p
  .locator('[data-celula="descricao"]', { hasText: "Energia elétrica" })
  .count();
...
ok(
  "proposta enviada avisa quem precisa aceitar",
...
ok(
  "quem propôs pode cancelar (o botão existe agora)",
...
ok(
  "o gestor que recebe vê a proposta na página do custo",
  paraAceitar.includes("aguardando aceite"),
);
const botaoAceite = p.locator("button", { hasText: /Aceitar/ });
ok(
  "o botão de aceitar mostra o valor em reais, não só o percentual",
...
ok(
  "o texto diz a verdade: recusar derruba a proposta inteira",
...
await p.locator('button[aria-label^="Ações de"]').first().click();
await p.waitForSelector('[role="menu"]');
const desabilitados = await p
  .locator('[role="menu"] [role="menuitem"][aria-disabled="true"]')
  .count();
...
ok(
  "e cada uma explica o motivo, em vez de ficar muda",
  textoMenu.includes("consultar"),
...
await p.keyboard.press("Escape");
await p.waitForTimeout(200);
const voltouAoGatilho = await p.evaluate(() =>
...
ok(
  "nenhum erro de JavaScript durante a sessão inteira",
  relevantes.length === 0,
